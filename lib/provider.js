 ...
 * @param {object} deps.backends     keyed backend registry (exa/parallel/ddg/deepseek/anthropic/openai)
 */
  ...
  return {
    ...
    async search(request, signal) {
      ...
      for (const id of opts.enabledBackends) {
        ...
        let avail;
        if (be.requiresCredential) {
          avail = be.available(opts.backends?.[id] ?? {});
        } else {
          avail = be.available();
        }
        if (!avail) continue;
        eligible.push({ backend: be, beOpts: be.requiresCredential ? (opts.backends?.[id] ?? {}) : {} });
      }
      ...
      const results = await Promise.allSettled(
        eligible.map(async ({ backend: be, beOpts }) => {
          const recordRequest = (req) => recordBackendRequest(ctx, be.id, req);
          const recordOutcome = (out) => recordBackendOutcome(ctx, be.id, out);
          const hooks = { recordRequest, recordOutcome };
          if (be.requiresCredential) {
            return be.search(request, signal, beOpts, hooks);
          }
          return be.search(request, signal, hooks);
        }),
      );
      ...
      if (merged.length > 0) {
        const truncated = merged.length >= maxResults && merged.length === results.length;
        // Re-truncate to maxResults after dedup (in case we filled early).
        const sources = merged.slice(0, maxResults);
        const wasCapped =
          merged.reduce((acc, s) => acc + 1, 0) > maxResults ||
          results.some((r) => r.status === "fulfilled" && (r.value?.sources?.length ?? 0) > maxResults);
        return { sources, content: undefined, truncated: sources.length >= maxResults && wasCapped };
      }
