...
export function recordBackendRequest(ctx, backendId, request) {
  try {
    ...
    if (session?.append) {
      session.append("web/unified-search-backend-request", { backendId, ...request, ignorable: true });
    }
  } catch {
...
export function recordBackendOutcome(ctx, backendId, outcome) {
  try {
    ...
    const session = initiator?.session;
    if (session?.append) {
      session.append("web/unified-search-backend-outcome", { backendId, ...outcome, ignorable: true });
    }
  } catch {
    // best-effort
  }
