...
/** Per-process session id - stable so Parallel free-tier rate limiting correlates calls. */
const SESSION_ID = "dsh-websearch-" + randomUUID().slice(0, 8);
...
export const parallelBackend = {
  ...
  async search(request, signal, options, hooks = {}) {
    ...
    if (result?.isError === true) {
      ...
      throw new WebError(`parallel returned an MCP error: ${message}`, "WEB_PROVIDER_ERROR");
