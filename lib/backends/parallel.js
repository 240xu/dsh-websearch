...
export const parallelBackend = {
  ...
  async search(request, signal, hooks = {}) {
    ...
    let result;
    try {
      result = await rpcCall(PARALLEL_ENDPOINT, PARALLEL_TOOL, args, {}, signal);
    } catch (err) {
...
/**
 * Parse Parallel's JSON-string text response into WebSearchSource[].
 * The body inside result.content[0].text is JSON-shaped:
 *   {
 *     "search_id": "...",
 *     "results": [
 ...
 *     ]
 *   }
 * excerpt[0] → snippet (may be long; we cap at 240 chars); publish_date ("null"
 * or missing) → publishedAt (omitted when literal "null"); title → title.
 *
...
export function parseParallelResult(result, maxResults) {
  ...
  try {
    ...
    return []; // not-JSON → no sources; surface silently (provider stays alive).
  }
  ...
  for (const r of results) {
    ...
    // publish_date of literal "null" → treat as no date.
