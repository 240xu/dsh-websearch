window.__ModuleLoader__.load({
	...
	factory: (require) => {
			...
			async save() {
				...
				try {
					for (const [field, staged] of this.staged) {
						if (staged.clear) await this.scope.unset(field);
						else {
							const num = Number(staged.text);
							await this.scope.set(field, GLOBAL_FIELDS.includes(field) && staged.text !== "" && !Number.isNaN(num) ? num : staged.text);
						}
					}
					...
					this.staged.clear(); this.stagedKeys.clear();
			...
			inject() {
				return {
					hooks: {
						...
						save: () => this.save(),
						discard: () => { this.staged.clear(); this.stagedKeys.clear(); this.republish(); },
					},
				};
			}
		}
		...
		function ToggleRow(props) {
			...
			const checked = props.value === true || (props.value === undefined && ["exa","parallel","ddg","searxng"].includes(b.id));
			...
			return react_jsx_runtime.jsxs("div", { className: "wsc-card", children: [
				...
				react_jsx_runtime.jsx("div", { className: "wsc-toggles", children: BACKENDS.map((b) =>
					react_jsx_runtime.jsx(ToggleRow, {
						...
						onChange: (checked) => card.edit(toggleField(b.id), checked ? "true" : ""),
					}, b.id)
				) }),
				...
				react_jsx_runtime.jsxs("div", { className: "wsc-grid", children: [
					react_jsx_runtime.jsxs("div", { children: [
						...
						react_jsx_runtime.jsx("input", { className: "wsc-input", value: state.stagedCount >= 0 ? (state.value.numResults ?? "") : "", onChange: e => card.edit("numResults", e.target.value), inputMode: "numeric" }),
					] }),
					react_jsx_runtime.jsxs("div", { children: [
						...
						react_jsx_runtime.jsx("input", { className: "wsc-input", value: state.value.concurrency ?? "", onChange: e => card.edit("concurrency", e.target.value), inputMode: "numeric" }),
					] }),
					react_jsx_runtime.jsxs("div", { children: [
						...
						react_jsx_runtime.jsx("input", { className: "wsc-input", value: state.value.backendTimeoutMs ?? "", onChange: e => card.edit("backendTimeoutMs", e.target.value), inputMode: "numeric" }),
					] }),
				] }),
				...
				react_jsx_runtime.jsx("div", { className: "wsc-grid", children: BACKENDS.filter((b) => b.keyEnv).map((b) =>
					react_jsx_runtime.jsxs("div", { children: [
						react_jsx_runtime.jsxs("div", { className: "wsc-fieldHead", children: [
							react_jsx_runtime.jsx("span", { className: "wsc-label", children: props.t.keyLabel(b.id) }),
