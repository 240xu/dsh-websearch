				...
				try {
					for (const [field, staged] of this.staged) {
						...
						if (SELECT_FIELDS[field]) {
							if (!SELECT_FIELDS[field].includes(staged.text)) { this.failed = true; continue; }
							await this.scope.set(field, staged.text);
							doneFields.push(field);
							continue;
						}
						if (BOOLEAN_FIELDS.includes(field)) {
							if (staged.text !== "true" && staged.text !== "false") { this.failed = true; continue; }
